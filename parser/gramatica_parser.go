...
const (
	...
	gramaticaParserNEWLINE = 7
	gramaticaParserINT     = 8
)

// gramaticaParser rules.
const (
	gramaticaParserRULE_prog = 0
	gramaticaParserRULE_expr = 1
)
...
func (s *ProgContext) AllNEWLINE() []antlr.TerminalNode {
	return s.GetTokens(gramaticaParserNEWLINE)
}

func (s *ProgContext) NEWLINE(i int) antlr.TerminalNode {
	return s.GetToken(gramaticaParserNEWLINE, i)
}
...
func (p *gramaticaParser) Prog() (localctx IProgContext) {
	...
	for _la == gramaticaParserT__4 || _la == gramaticaParserINT {
		{
			p.SetState(4)
			p.expr(0)
		}
		{
			p.SetState(5)
			p.Match(gramaticaParserNEWLINE)
			if p.HasError() {
...
func (p *gramaticaParser) expr(_p int) (localctx IExprContext) {
	...
	_startState := 2
	p.EnterRecursionRule(localctx, 2, gramaticaParserRULE_expr, _p)
	var _la int
